...
class DDPG(object):
    def __init__(self, a_dim, s_dim, a_bound,load):
        ...
        if load:            
            ...
            self.Actor_eval.load_state_dict(torch.load('actor_eval.pkl'))
            self.Critic_eval.load_state_dict(torch.load('critic_eval.pkl'))
            self.Actor_target.load_state_dict(torch.load('actor_target.pkl'))
            self.Critic_target.load_state_dict(torch.load('critic_target.pkl'))
    ...
    def save_model(self):
        torch.save(self.Actor_eval.state_dict(), 'actor_eval.pkl')
        torch.save(self.Critic_eval.state_dict(),'critic_eval.pkl')
        torch.save(self.Actor_target.state_dict(), 'actor_target.pkl')
        torch.save(self.Critic_target.state_dict(),'critic_target.pkl')
...
env = Wave()
env.seed(9998)
s_dim = 2#env.observation_space.shape[0]
...
Offset_y = []
load = 0
ddpg = DDPG(a_dim, s_dim, a_bound,load)
...
for i in range(MAX_EPISODES):
    ...
    for j in range(MAX_EP_STEPS):
        ...
        ep_reward += r
        if j == MAX_EP_STEPS-1:
            running_reward = reward_sum if running_reward is None else running_reward * 0.99 + ep_reward * 0.01
            print('REINFORCE ep %03d done. reward: %f. reward running mean: %f. Explore: %.2f' % (i, ep_reward, running_reward,var))
            reward_save.append(ep_reward)
